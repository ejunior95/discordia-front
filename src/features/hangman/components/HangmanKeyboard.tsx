      ...
      {LETTERS.map((l) => {
        ...
        return (
          <Button
            ...
            onClick={() => onGuess(l)}
            className={cn(
              "h-10 sm:h-11 md:h-12 p-0 text-sm sm:text-base font-bold cursor-pointer select-none",
              "transition-colors",
              isCorrect && "bg-emerald-500/20 border-emerald-500 text-emerald-600 dark:text-emerald-400",
              isWrong && "bg-destructive/20 border-destructive text-destructive opacity-80",
            )}
          >
            {l}
          </Button>
